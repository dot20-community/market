...
async function getBalance(address: string): Promise<({balance: BalanceInfo[]})> {
...
export function Account() {
  ...
  return (
    <div>
        ...
        <Table className="w-2/3">
          ...
          <TableBody>
            <TableRow key="1">
              ...
              <TableCell>{dotaBalance.toString().replace(/\B(?=(\d{3})+(?!\d))/g, ",")}</TableCell>
              <TableCell>
      ...
      <SellModal dotaBalance={dotaBalance} isOpen={isOpen} onOpenChange={onOpenChange}/>
    </div>
  );
}